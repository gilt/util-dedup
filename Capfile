...
role :rails_server, 'weba1', 'weba2', 'weba3', 'web-rails1', 'web-rails2', 'web-rails3', 'web-rails4'
role :deploy_code, 'irb1'

ALL_ROLES = [:rails_server, :job_worker, :warehouse_server, :deploy_code]
  ...
  def run
    ...
    threads << Thread.new do
      ...
      HostSplitter.new(all_hosts).each do |hosts|
        ...
        # a few seconds to let zeus pool bring back the nodes we just restarted
        sleep 5
      end
    end
